...
const os = [
  ...
  rt = 366,
  productIds = [32680,32673,32661,32660,32651,32646,32637,32635,32629,32621,32614,32607,32599,32592,32583,32530,32426,32288,32238,32184,32177,32166,32160,32154,32148,32136,32130,32071,32044,32040,32032,32031,32024,32017,32014,31988,31986,31978,31977,31974,31971,31967,31942,31938,31933,31929,31922,31921,31916,23774,22430,27283,22394,16755,31723,31710,31698,31688,31671,31654,31649,31639,23854,31621,31609,31604,26608,15926,31586,31567,31545,31536,27452,31524,31514,27260,31508,31501,22205,22180,31482,31464,23175,27440,31595,31456,31431,27253,31416,22839,26667,27427,22134,27247,31349,31342,31320,31319,31310,31287,31278,31270,31269,31261,31248,31241,31234,31217,31215,31197,31189,31180,31173,31156,27270,31065,31021,31056,31047,31039,31030,31012,31002,30993,30835,30982,30975,30803,30968,30958,30948,30841,30939,30821,30797,30930,30815,30809,30753,30788,30865,30759,30780,30738,30745,30731,30721,30715,30708,30702,30696,30683,30764,30690,30659,30648,30566,30361,28309,27853,27459,27419,27177,26628,26623,26614,26594,26592,26585,26577,26570,32067,31962,23866,23844,23831,23821,23812,23805,23790,23781,23243,23234,23225,23203,23186,23161,23149,23139,23128,23090,23077,23015,23005,22830,22811,22797,22790,22762,22749,22688,22679,22667,22656,22641,22634,22619,22589,22579,22563,22544,22514,22505,22490,22481,22453,22441,22416,22405,22386,22370,22349,22326,22315,22304,22294,22271,22260,22247,22235,22225,22213,22194,22103,22089,22078,21737,21720,21644,21627,21565,21461,21411,20902,16952,16945,16938,16767,16746,16908,16906,16885,16790,16738,16718,16709,16681,16611,15914,15901,15841,15779,15997,15974,15919,15905,15889,15876,15864,15858,15853,15847,15835,15829,15817,15811,15795,15790,15784,15773,15767,15761,15754,15747,15694,15688,15676,15671,15667],
  ss = 1.1,
  as = 1.6,
  Mn = 0.8,
  Ht = 0.04,
...
class n0 {
  ...
  injectStyles() {
    ...
    ((t.textContent = `
      ...
      /* ── Close (×) — centered on image overlay ── */
      .dp-close-btn {
        position: fixed;
        top: 50%;
        left: 50%;
  ...
  setContent(t) {
    ...
    if (n % 5 === 0) {
      ...
      const productId = productIds[p - 1];
      const link = productId ? `https://minalidya.wedding/single-product?id=${productId}&lang=tr` : "#";
      this.labelInner.innerHTML = `<span class="dp-label-day">${n}. Gün&ensp;|&ensp;</span><a href="${link}" target="_blank" class="dp-label-title-link">${firstWord}</a>`;
    }
  }
  makeClose() {
